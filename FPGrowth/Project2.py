...
def data_reorder(data, item_freq):
...
def fp_tree_create_and_update(init_node, trans, similar_item_dict):
    ...
    # Else, create a new node for the child and link it to its parent
        ...
        init_node.child[trans[0]] = TreeNode(trans[0], 1, init_node)
...
def conditional_tree(name, init_node, data):
    if data[0][0] == name:
        # Skip the conditional tree if no extra frequent items are occuring
        if len(data) > 1:
            conditional_tree(name, init_node, data[1::])
    if data[0][0] != name:
        ...
        # Else, create a new child node and update its frequency
            ...
            init_node.child[data[0][0]] = TreeNode(data[0][0], data[0][1], \
                                                   init_node)
        # Continue to recursively create the conditional FP-Tree for each item
        if len(data) > 1:
            conditional_tree(name, init_node.child[data[0][0]], data[1::])
...
def create_CFP(similar_item_dict, threshold):
    final_cond_base = []
    ...
    for key, value in similar_item_dict.items():
        ...
        c_leaf = key
        c_root = TreeNode('Null Set', 1, None)
        # Creates the conditional tree from the above conditonal pattern base
        for row in final_cond_base_key:
            conditional_tree(c_leaf, c_root, row)
        # Prints the conditional tree in nested list format if height > 1
        if len(c_root.child) > 1:
            c_root.display()

        ## Ending code for question 2B
    # Removes any duplicate rows having the same itemset
    unique_set = set(map(tuple, final_cond_base))
    unique_list = list(unique_set)
    # Sorts the list in alphabetical order for better readibility
    unique_list.sort(key=lambda \
            unique_cond_base_list: unique_cond_base_list[0])
    unique_cond_base = map(list, unique_list)
    save_to_file(unique_cond_base)  # Exports the asnwer to a CSV file



# Required User Inputs
support =2     #have to choose moderate value if not enogh frequency the program will show error

file_name = 'S.csv'  #data set

file_delimiter = ','   #this has to be changed according to the data set decription(for most cases its ',')

output_file_name = "output.csv"    #most frequent items will be stored in this file
...
"""
In header table address of each node is stored
That is used to find most frequent items.
"""
fptree_root, header_table = data_reorder(dataset, freq_items)


"""
Now,from Conditional FP tree Frequent Patterns are Generated
"""
create_CFP(header_table, support)
