...
#print(X_test)
"""
checking for maximum accuracy for different tree depths
"""
"""
start = t()
...
classifier = DecisionTreeClassifier(min_samples_split=5, max_depth=7)
classifier.fit(X_train,Y_train)
...
for i in range(1,15):
    ...
    start = t()
    process = psutil.Process(os.getpid())
    classifier = DecisionTreeClassifier(min_samples_split=5, max_depth=i)
    classifier.fit(X_train,Y_train)
...
print("time:",time)
print("memory",memory)
print("max_accuracy= ",max,"for tree depth=",index)
